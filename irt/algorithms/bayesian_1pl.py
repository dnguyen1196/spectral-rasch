...
class OneParamLog(IrtModel):
    ...
    def fit(self, models, items, responses, num_epochs=10000, eps=1e-3):
        ...
        for j in range(num_epochs):
            next_loss = svi.step(models, items, responses)
            if np.abs(next_loss - loss)/np.abs(loss) < eps and next_loss < loss:
                break
            loss = next_loss
